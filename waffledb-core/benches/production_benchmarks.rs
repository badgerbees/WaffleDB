...
use std::io::Write;
    ...
    async fn benchmark_end_to_end_ingestion_pipeline() {
        ...
        // Success criteria: within 20% of WAL benchmark (100K vecs/sec)
        let min_throughput = 80000.0; // 80% of 100K
        assert!(
            throughput >= min_throughput,
            "End-to-end throughput {} below minimum {}", throughput, min_throughput
        );

        println!("\n✅ PASSED: End-to-end throughput within acceptable range");
    }
    ...
    async fn benchmark_concurrent_reads_during_snapshot() {
        ...
        // Wait for everything
        task::JoinSet::from_iter(tasks).join_all().await;
    ...
    fn benchmark_memory_usage_during_snapshots() {
        ...
        // Phase 1: Allocate vectors
        {
            ...
            let vectors = vec![vec![0.0f32; vector_dims]; vector_count];
    ...
    fn benchmark_snapshot_compression_ratios() {
        ...
        // Test 1: Uniform data (best case for compression)
        {
            ...
            let data = vec![0.42f32; 1_000_000];
...
fn main() {
    println!("Production Benchmarks - Run with: cargo test --release --lib -- --nocapture");
}
